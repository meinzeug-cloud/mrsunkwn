...
class FamilyDB(Base):
    """SQLAlchemy model for Family in database"""
    ...
    priority = Column(String(20), default=FamilyPriority.MEDIUM.value)
    ...
    # Relationships
    user = relationship("User", back_populates="familys")
    family = relationship("Family", back_populates="familys")

# Pydantic Models (API)
class FamilyBase(BaseModel):
    """Base model for Family API operations"""
    ...
    priority: FamilyPriority = Field(default=FamilyPriority.MEDIUM)
...
class FamilyCreate(FamilyBase):
    """Model for creating new Family"""
    user_id: str = Field(..., description="User ID who owns this family")
    ...
    class Config:
        schema_extra = {
            "example": {
                "name": "Sample Family",
                "description": "A sample family for demonstration",
                "status": "active",
                "priority": "medium",
...
class FamilyUpdate(BaseModel):
    ...
    priority: Optional[FamilyPriority] = None
...
class FamilyInDB(FamilyBase):
    """Model for Family as stored in database"""
...
class FamilyList(BaseModel):
    """Model for paginated Family list response"""
    items: List[FamilyResponse] = Field(..., description="List of familys")
...
class FamilyAnalytics(BaseModel):
    """Model for Family analytics data"""
    user_id: str
    family_id: str
    timeframe: str
    total_items: int
...
class FamilyOperations:
    """Database operations for Family"""
    
    def __init__(self, db):
        self.db = db
    
    async def create(self, data: FamilyCreate) -> FamilyInDB:
        ...
        return FamilyInDB.from_orm(db_item)
    
    async def get_by_id(self, item_id: str) -> Optional[FamilyInDB]:
        """Get family by ID"""
        db_item = await self.db.query(FamilyDB).filter(
            FamilyDB.id == item_id
        ).first()
        
        return FamilyInDB.from_orm(db_item) if db_item else None
    
    async def get_by_user(self, user_id: str, page: int = 1, per_page: int = 20) -> FamilyList:
        """Get familys by user ID"""
        offset = (page - 1) * per_page
        
        query = self.db.query(FamilyDB).filter(
            FamilyDB.user_id == user_id
        )
        ...
        return FamilyList(
            items=[FamilyResponse.from_orm(item) for item in items],
            total=total,
            page=page,
            per_page=per_page,
    ...
    async def update(self, item_id: str, data: FamilyUpdate) -> Optional[FamilyInDB]:
        """Update family by ID"""
        db_item = await self.db.query(FamilyDB).filter(
            FamilyDB.id == item_id
        ).first()
    ...
    async def get_analytics(self, user_id: str, timeframe: str = "week") -> FamilyAnalytics:
        """Get analytics for familys"""
