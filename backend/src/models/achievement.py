...
class AchievementPriority(str, Enum):
    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"
    CRITICAL = "critical"
...
class AchievementDB(Base):
    """SQLAlchemy model for Achievement in database"""
    ...
    priority = Column(String(20), default=AchievementPriority.MEDIUM.value)
...
class AchievementBase(BaseModel):
    """Base model for Achievement API operations"""
    name: str = Field(..., min_length=1, max_length=255, description="Name of the achievement")
    description: Optional[str] = Field(None, max_length=1000, description="Description")
    status: MrsUnkwnStatus = Field(default=MrsUnkwnStatus.ACTIVE)
    priority: AchievementPriority = Field(default=AchievementPriority.MEDIUM)
...
class AchievementCreate(AchievementBase):
    """Model for creating new Achievement"""
    user_id: str = Field(..., description="User ID who owns this achievement")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "description": "A sample achievement for demonstration",
                "status": "active",
                "priority": "medium",
...
class AchievementUpdate(BaseModel):
    """Model for updating Achievement"""
    name: Optional[str] = Field(None, min_length=1, max_length=255)
    description: Optional[str] = Field(None, max_length=1000)
    status: Optional[MrsUnkwnStatus] = None
    priority: Optional[AchievementPriority] = None
...
class AchievementInDB(AchievementBase):
    """Model for Achievement as stored in database"""
...
class AchievementResponse(AchievementInDB):
    """Model for Achievement API response"""
...
class AchievementList(BaseModel):
    """Model for paginated Achievement list response"""
    items: List[AchievementResponse] = Field(..., description="List of achievements")
...
class AchievementAnalytics(BaseModel):
    """Model for Achievement analytics data"""
    user_id: str
    family_id: str
    timeframe: str
    total_items: int
...
class AchievementOperations:
    """Database operations for Achievement"""
    
    def __init__(self, db):
        self.db = db
    
    async def create(self, data: AchievementCreate) -> AchievementInDB:
        """Create new achievement in database"""
        import uuid
        
        db_item = AchievementDB(
            id=str(uuid.uuid4()),
            **data.dict(),
        ...
        return AchievementInDB.from_orm(db_item)
    
    async def get_by_id(self, item_id: str) -> Optional[AchievementInDB]:
        """Get achievement by ID"""
        db_item = await self.db.query(AchievementDB).filter(
            AchievementDB.id == item_id
        ).first()
        
        return AchievementInDB.from_orm(db_item) if db_item else None
    
    async def get_by_user(self, user_id: str, page: int = 1, per_page: int = 20) -> AchievementList:
        """Get achievements by user ID"""
        offset = (page - 1) * per_page
        
        query = self.db.query(AchievementDB).filter(
        ...
        return AchievementList(
            items=[AchievementResponse.from_orm(item) for item in items],
            total=total,
            page=page,
            per_page=per_page,
    ...
    async def update(self, item_id: str, data: AchievementUpdate) -> Optional[AchievementInDB]:
    ...
    async def get_analytics(self, user_id: str, timeframe: str = "week") -> AchievementAnalytics:
        """Get analytics for achievements"""
