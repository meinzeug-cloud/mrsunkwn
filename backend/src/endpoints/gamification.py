...
class GamificationBase(BaseModel):
    """Base model for gamification with Mrs-Unkwn specific fields"""
...
class GamificationCreate(GamificationBase):
    """Model for creating gamification with Mrs-Unkwn features"""
...
@router.get(
    "/",
    response_model=List[GamificationResponse],
    summary="Get gamifications for family",
    description="Retrieve family's gamifications with parental filtering"
)
async def get_gamifications(
    ...
    """Get gamifications with Mrs-Unkwn family filtering and analytics"""
    try:
        # Log access attempt
        ...
        items = await service.get_filtered_gamifications(
            filters=filters,
        ...
        logger.info(f"Retrieved {len(items)} gamifications for user {current_user.id}")
        return items
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error retrieving gamifications: {str(e)}")
...
@router.post(
    ...
    summary="Create new gamification",
    description="Create new gamification with AI tutor integration"
)
async def create_gamification(
    request: GamificationCreate,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user),
    background_tasks: BackgroundTasks = BackgroundTasks()
):
    """Create new gamification with Mrs-Unkwn features"""
    try:
        ...
        service = GamificationService(db)
        new_item = await service.create_gamification(request, current_user.id)
        
        # Initialize AI tutor if enabled
        if request.ai_interaction_enabled:
            ai_service = AITutorService()
            await ai_service.initialize_for_gamification(new_item.id)
        ...
        logger.info(f"Created gamification {new_item.id} for user {current_user.id}")
        return new_item
...
@router.get(
    "/{item_id}",
    response_model=GamificationResponse,
    summary="Get gamification by ID",
    description="Get specific gamification with real-time monitoring data"
)
async def get_gamification(
    item_id: str = Path(..., description="ID of the gamification"),
    include_live_data: bool = Query(True, description="Include real-time monitoring data"),
    current_user = Depends(get_current_user),
    db: Session = Depends(get_db),
    background_tasks: BackgroundTasks = BackgroundTasks()
):
    """Get gamification with Mrs-Unkwn monitoring integration"""
    try:
        # Verify access
        ...
        # Get through service
        service = GamificationService(db)
        item = await service.get_gamification_by_id(item_id, include_live_data)
        ...
        logger.error(f"Error retrieving gamification {item_id}: {str(e)}")
...
@router.post(
    ...
    description="Interact with AI tutor for this gamification"
)
...
@router.get(
    ...
    description="Get comprehensive learning analytics for this gamification"
)
