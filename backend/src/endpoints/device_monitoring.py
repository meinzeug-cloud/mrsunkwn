...
from models.device_monitoring import Device_Monitoring
from services.device_monitoring_service import Device_MonitoringService
...
class Device_MonitoringBase(BaseModel):
    """Base model for device_monitoring with Mrs-Unkwn specific fields"""
...
class Device_MonitoringCreate(Device_MonitoringBase):
    """Model for creating device_monitoring with Mrs-Unkwn features"""
...
class Device_MonitoringResponse(Device_MonitoringBase):
...
@router.get(
    "/",
    response_model=List[Device_MonitoringResponse],
    summary="Get device monitorings for family",
    description="Retrieve family's device monitorings with parental filtering"
)
async def get_device_monitorings(
    ...
    """Get device_monitorings with Mrs-Unkwn family filtering and analytics"""
    try:
        # Log access attempt
        await log_learning_activity(
            current_user.id, 
            "view_device_monitorings", 
        ...
        service = Device_MonitoringService(db)
        items = await service.get_filtered_device_monitorings(
            filters=filters,
        ...
        logger.info(f"Retrieved {len(items)} device_monitorings for user {current_user.id}")
        return items
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error retrieving device_monitorings: {str(e)}")
        raise HTTPException(status_code=500, detail="Error retrieving data")

@router.post(
    "/",
    response_model=Device_MonitoringResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Create new device monitoring",
    description="Create new device monitoring with AI tutor integration"
)
async def create_device_monitoring(
    request: Device_MonitoringCreate,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user),
    background_tasks: BackgroundTasks = BackgroundTasks()
):
    """Create new device_monitoring with Mrs-Unkwn features"""
    try:
        ...
        service = Device_MonitoringService(db)
        new_item = await service.create_device_monitoring(request, current_user.id)
        
        # Initialize AI tutor if enabled
        if request.ai_interaction_enabled:
            ai_service = AITutorService()
            await ai_service.initialize_for_device_monitoring(new_item.id)
        ...
        logger.info(f"Created device_monitoring {new_item.id} for user {current_user.id}")
        return new_item
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error creating device_monitoring: {str(e)}")
        raise HTTPException(status_code=500, detail="Error creating resource")

@router.get(
    "/{item_id}",
    response_model=Device_MonitoringResponse,
    summary="Get device monitoring by ID",
    description="Get specific device monitoring with real-time monitoring data"
)
async def get_device_monitoring(
    item_id: str = Path(..., description="ID of the device_monitoring"),
    include_live_data: bool = Query(True, description="Include real-time monitoring data"),
    current_user = Depends(get_current_user),
    db: Session = Depends(get_db),
    background_tasks: BackgroundTasks = BackgroundTasks()
):
    """Get device_monitoring with Mrs-Unkwn monitoring integration"""
    try:
        # Verify access
        ...
        # Get through service
        service = Device_MonitoringService(db)
        item = await service.get_device_monitoring_by_id(item_id, include_live_data)
        
        if not item:
            raise HTTPException(status_code=404, detail="Device_Monitoring not found")
        ...
        logger.error(f"Error retrieving device_monitoring {item_id}: {str(e)}")
...
@router.post(
    ...
    description="Interact with AI tutor for this device_monitoring"
)
...
@router.get(
    ...
    description="Get comprehensive learning analytics for this device_monitoring"
)
