...
from models.parental_controls import Parental_Controls
from services.parental_controls_service import Parental_ControlsService
...
class Parental_ControlsBase(BaseModel):
    """Base model for parental_controls with Mrs-Unkwn specific fields"""
...
class Parental_ControlsCreate(Parental_ControlsBase):
    """Model for creating parental_controls with Mrs-Unkwn features"""
...
class Parental_ControlsResponse(Parental_ControlsBase):
...
@router.get(
    "/",
    response_model=List[Parental_ControlsResponse],
    summary="Get parental controlss for family",
    description="Retrieve family's parental controlss with parental filtering"
)
async def get_parental_controlss(
    ...
    """Get parental_controlss with Mrs-Unkwn family filtering and analytics"""
    try:
        # Log access attempt
        await log_learning_activity(
            current_user.id, 
            "view_parental_controlss", 
        ...
        service = Parental_ControlsService(db)
        items = await service.get_filtered_parental_controlss(
            filters=filters,
        ...
        logger.info(f"Retrieved {len(items)} parental_controlss for user {current_user.id}")
        return items
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error retrieving parental_controlss: {str(e)}")
        raise HTTPException(status_code=500, detail="Error retrieving data")

@router.post(
    "/",
    response_model=Parental_ControlsResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Create new parental controls",
    description="Create new parental controls with AI tutor integration"
)
async def create_parental_controls(
    request: Parental_ControlsCreate,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user),
    background_tasks: BackgroundTasks = BackgroundTasks()
):
    """Create new parental_controls with Mrs-Unkwn features"""
    try:
        # Check parental controls if this is for a student
        if request.student_id:
            allowed = await check_parental_controls(
                request.student_id, 
                "create_parental_controls",
                db
            )
            if not allowed:
                raise HTTPException(
        ...
        service = Parental_ControlsService(db)
        new_item = await service.create_parental_controls(request, current_user.id)
        
        # Initialize AI tutor if enabled
        if request.ai_interaction_enabled:
            ai_service = AITutorService()
            await ai_service.initialize_for_parental_controls(new_item.id)
        ...
        logger.info(f"Created parental_controls {new_item.id} for user {current_user.id}")
        return new_item
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error creating parental_controls: {str(e)}")
        raise HTTPException(status_code=500, detail="Error creating resource")

@router.get(
    "/{item_id}",
    response_model=Parental_ControlsResponse,
    summary="Get parental controls by ID",
    description="Get specific parental controls with real-time monitoring data"
)
async def get_parental_controls(
    item_id: str = Path(..., description="ID of the parental_controls"),
    include_live_data: bool = Query(True, description="Include real-time monitoring data"),
    current_user = Depends(get_current_user),
    db: Session = Depends(get_db),
    background_tasks: BackgroundTasks = BackgroundTasks()
):
    """Get parental_controls with Mrs-Unkwn monitoring integration"""
    try:
        # Verify access
        ...
        # Get through service
        service = Parental_ControlsService(db)
        item = await service.get_parental_controls_by_id(item_id, include_live_data)
        
        if not item:
            raise HTTPException(status_code=404, detail="Parental_Controls not found")
        ...
        logger.error(f"Error retrieving parental_controls {item_id}: {str(e)}")
...
@router.post(
    ...
    description="Interact with AI tutor for this parental_controls"
)
...
@router.get(
    ...
    description="Get comprehensive learning analytics for this parental_controls"
)
