...
from models.parentalcontrol import (
    ParentalControlCreate,
    ParentalControlUpdate, 
    ParentalControlInDB,
    ParentalControlOperations,
    ParentalControlAnalytics
)
...
class ParentalControlService:
    """
    Mrs-Unkwn ParentalControlService - Comprehensive service for parentalcontrol management
    
    This service handles all business logic related to parentalcontrols
    in the Mrs-Unkwn platform, including AI integration and parental controls.
    """
    
    def __init__(self, db: Session):
        self.db = db
        self.operations = ParentalControlOperations(db)
        self.notification_service = NotificationService()
        self.ai_tutor_service = AITutorService(db)
        
    async def create_parentalcontrol(
        self, 
        data: ParentalControlCreate, 
        user_id: str
    ) -> ParentalControlInDB:
        """Create a new parentalcontrol with Mrs-Unkwn features"""
        try:
            logger.info(f"Creating parentalcontrol for user {user_id}: {data.name}")
            
            # Validate user permissions
            if not await self._validate_user_permissions(user_id, "create"):
                raise ValueError("User does not have permission to create parentalcontrols")
            ...
            if data.requires_parent_approval:
                ...
                if not parent_approved:
                    raise ValueError("Parental approval required for this parentalcontrol")
            
            # Create the parentalcontrol
            result = await self.operations.create(data)
            
            # Initialize AI tutor if enabled
            if data.ai_interaction_enabled:
                await self.ai_tutor_service.initialize_for_parentalcontrol(result.id)
            
            # Log activity
            await log_user_activity(
                user_id, 
                "create_parentalcontrol", 
            ...
            logger.info(f"ParentalControl created successfully: {result.id}")
            return result
            
        except Exception as e:
            logger.error(f"Error creating parentalcontrol: {str(e)}")
            raise
    
    async def get_parentalcontrol_by_id(
        self, 
        item_id: str, 
        user_id: str,
        include_analytics: bool = True
    ) -> Optional[ParentalControlInDB]:
        """Get parentalcontrol by ID with permission checks"""
        try:
            logger.info(f"Fetching parentalcontrol {item_id} for user {user_id}")
            ...
            if not await self._validate_access_permissions(user_id, item):
                raise ValueError("User does not have access to this parentalcontrol")
            ...
            logger.error(f"Error fetching parentalcontrol: {str(e)}")
            raise
    
    async def get_user_parentalcontrols(
        self,
        user_id: str,
        filters: Dict[str, Any] = None,
        page: int = 1,
        per_page: int = 20
    ) -> List[ParentalControlInDB]:
        """Get parentalcontrols for a user with filtering"""
        try:
            logger.info(f"Fetching parentalcontrols for user {user_id}")
            ...
            logger.error(f"Error fetching user parentalcontrols: {str(e)}")
            raise
    
    async def update_parentalcontrol(
        self,
        item_id: str,
        data: ParentalControlUpdate,
        user_id: str
    ) -> Optional[ParentalControlInDB]:
        """Update parentalcontrol with permission checks"""
        try:
            logger.info(f"Updating parentalcontrol {item_id} for user {user_id}")
            ...
            if not await self._validate_update_permissions(user_id, existing):
                raise ValueError("User does not have permission to update this parentalcontrol")
            ...
            if data.ai_interaction_enabled is not None:
                if data.ai_interaction_enabled:
                    await self.ai_tutor_service.initialize_for_parentalcontrol(item_id)
                else:
                    await self.ai_tutor_service.disable_for_parentalcontrol(item_id)
            
            # Log activity
            await log_user_activity(
                user_id,
                "update_parentalcontrol",
            ...
            logger.info(f"ParentalControl updated successfully: {item_id}")
            return result
            
        except Exception as e:
            logger.error(f"Error updating parentalcontrol: {str(e)}")
            raise
    
    async def delete_parentalcontrol(
        self,
        item_id: str,
        user_id: str,
        force: bool = False
    ) -> bool:
        """Delete parentalcontrol with permission checks"""
        try:
            logger.info(f"Deleting parentalcontrol {item_id} for user {user_id}")
            ...
            if not await self._validate_delete_permissions(user_id, existing):
                raise ValueError("User does not have permission to delete this parentalcontrol")
            ...
            await self.ai_tutor_service.cleanup_for_parentalcontrol(item_id)
            ...
            if success:
                ...
                logger.info(f"ParentalControl deleted successfully: {item_id}")
            
            return success
            
        except Exception as e:
            logger.error(f"Error deleting parentalcontrol: {str(e)}")
            raise
    ...
    async def get_analytics(
        ...
        """Get comprehensive analytics for user's parentalcontrols"""
        try:
            ...
            await log_user_activity(
                user_id,
                "view_analytics",
                {"timeframe": timeframe, "type": "parentalcontrol_analytics"}
            )
    ...
    async def _validate_access_permissions(self, user_id: str, item: ParentalControlInDB) -> bool:
    ...
    async def _validate_update_permissions(self, user_id: str, item: ParentalControlInDB) -> bool:
        """Validate user can update specific item"""
        return await self._validate_access_permissions(user_id, item)
    
    async def _validate_delete_permissions(self, user_id: str, item: ParentalControlInDB) -> bool:
    ...
    async def _check_parental_approval(self, user_id: str, data: ParentalControlCreate) -> bool:
        """Check if parental approval is granted"""
        # Implementation would check parental control settings
        return True
    
    async def _check_parental_approval_for_update(self, user_id: str, existing: ParentalControlInDB, data: ParentalControlUpdate) -> bool:
        """Check parental approval for updates"""
        return True
    
    async def _check_parental_approval_for_deletion(self, user_id: str, existing: ParentalControlInDB) -> bool:
        """Check parental approval for deletion"""
        return True
    
    async def _requires_parental_approval_for_update(self, existing: ParentalControlInDB, data: ParentalControlUpdate) -> bool:
    ...
    async def _notify_family_of_creation(self, item: ParentalControlInDB):
        """Notify family members of new item creation"""
        await self.notification_service.notify_family(
            item.family_id,
            f"New parentalcontrol created: {item.name}",
            {"type": "creation", "item_id": item.id}
        )
    
    async def _notify_family_of_update(self, item: ParentalControlInDB, changes: ParentalControlUpdate):
        ...
        if any(getattr(changes, field, None) is not None for field in ['difficulty_level', 'subject_areas']):
            await self.notification_service.notify_family(
                item.family_id,
                f"ParentalControl updated: {item.name}",
                {"type": "update", "item_id": item.id, "changes": changes.dict(exclude_unset=True)}
            )
    
    async def _notify_family_of_deletion(self, item: ParentalControlInDB):
        """Notify family members of item deletion"""
        await self.notification_service.notify_family(
            item.family_id,
            f"ParentalControl deleted: {item.name}",
            {"type": "deletion", "item_id": item.id}
        )
    
    async def _enhance_analytics_with_ai_insights(self, analytics: ParentalControlAnalytics) -> ParentalControlAnalytics:
