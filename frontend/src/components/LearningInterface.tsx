...
export const LearningInterface: React.FC<LearningInterfaceProps> = ({
  ...
  // State Management
  const [state, setState] = useState<LearningInterfaceState>({
    expanded: true,
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`learninginterface_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/learninginterfaces/user/${userId}` : `/api/learninginterfaces`;
  ...
  useEffect(() => {
    if (error) {
      console.error(`LearningInterface error:`, error);
      onError?.(error);
    }
  }, [error, onError]);

  // Event Handlers
  const updateState = useCallback((updates: Partial<LearningInterfaceState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `learninginterface_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `learninginterface_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading learninginterfaces...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading learninginterfaces</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No learninginterfaces found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first learninginterface'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`learninginterface-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="learninginterface-component"
    >
