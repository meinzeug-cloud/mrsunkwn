...
export const SearchBar: React.FC<SearchBarProps> = ({
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`searchbar_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/searchbars/user/${userId}` : `/api/searchbars`;
  ...
  // Event Handlers
  const updateState = useCallback((updates: Partial<SearchBarState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `searchbar_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `searchbar_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading searchbars...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading searchbars</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No searchbars found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first searchbar'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`searchbar-component theme-${theme} size-${size} variant-${variant} ${className}`}
