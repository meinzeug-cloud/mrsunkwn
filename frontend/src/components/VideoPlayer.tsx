...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`videoplayer_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/videoplayers/user/${userId}` : `/api/videoplayers`;
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `videoplayer_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `videoplayer_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading videoplayers...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading videoplayers</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No videoplayers found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first videoplayer'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`videoplayer-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="videoplayer-component"
    >
