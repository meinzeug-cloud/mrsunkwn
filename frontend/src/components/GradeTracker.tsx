...
export const GradeTracker: React.FC<GradeTrackerProps> = ({
  ...
  // State Management
  const [state, setState] = useState<GradeTrackerState>({
    expanded: true,
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`gradetracker_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/gradetrackers/user/${userId}` : `/api/gradetrackers`;
  ...
  useEffect(() => {
    if (error) {
      console.error(`GradeTracker error:`, error);
      onError?.(error);
    }
  }, [error, onError]);

  // Event Handlers
  const updateState = useCallback((updates: Partial<GradeTrackerState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `gradetracker_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `gradetracker_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading gradetrackers...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading gradetrackers</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No gradetrackers found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first gradetracker'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`gradetracker-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="gradetracker-component"
    >
