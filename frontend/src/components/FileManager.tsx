...
export const FileManager: React.FC<FileManagerProps> = ({
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`filemanager_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/filemanagers/user/${userId}` : `/api/filemanagers`;
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `filemanager_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `filemanager_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading filemanagers...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading filemanagers</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No filemanagers found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first filemanager'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`filemanager-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="filemanager-component"
    >
