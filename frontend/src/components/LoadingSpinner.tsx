...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`loadingspinner_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/loadingspinners/user/${userId}` : `/api/loadingspinners`;
  ...
  // Event Handlers
  const updateState = useCallback((updates: Partial<LoadingSpinnerState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `loadingspinner_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `loadingspinner_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading loadingspinners...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading loadingspinners</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No loadingspinners found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first loadingspinner'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`loadingspinner-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="loadingspinner-component"
    >
