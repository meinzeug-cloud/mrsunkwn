...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`notificationcenter_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/notificationcenters/user/${userId}` : `/api/notificationcenters`;
  ...
  // Event Handlers
  const updateState = useCallback((updates: Partial<NotificationCenterState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `notificationcenter_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `notificationcenter_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading notificationcenters...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading notificationcenters</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No notificationcenters found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first notificationcenter'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`notificationcenter-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="notificationcenter-component"
    >
