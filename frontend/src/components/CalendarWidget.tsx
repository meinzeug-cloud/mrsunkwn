...
export const CalendarWidget: React.FC<CalendarWidgetProps> = ({
  ...
  // State Management
  const [state, setState] = useState<CalendarWidgetState>({
    expanded: true,
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`calendarwidget_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/calendarwidgets/user/${userId}` : `/api/calendarwidgets`;
  ...
  // Event Handlers
  const updateState = useCallback((updates: Partial<CalendarWidgetState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `calendarwidget_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `calendarwidget_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading calendarwidgets...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading calendarwidgets</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No calendarwidgets found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first calendarwidget'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`calendarwidget-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="calendarwidget-component"
    >
