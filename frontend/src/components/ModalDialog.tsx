...
export const ModalDialog: React.FC<ModalDialogProps> = ({
  ...
  // State Management
  const [state, setState] = useState<ModalDialogState>({
    expanded: true,
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`modaldialog_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/modaldialogs/user/${userId}` : `/api/modaldialogs`;
  ...
  useEffect(() => {
    if (error) {
      console.error(`ModalDialog error:`, error);
      onError?.(error);
    }
  }, [error, onError]);

  // Event Handlers
  const updateState = useCallback((updates: Partial<ModalDialogState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `modaldialog_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `modaldialog_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading modaldialogs...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading modaldialogs</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No modaldialogs found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first modaldialog'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`modaldialog-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="modaldialog-component"
    >
