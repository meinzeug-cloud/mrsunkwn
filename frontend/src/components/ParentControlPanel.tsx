...
interface ParentControlPanelState {
  expanded: boolean;
...
export const ParentControlPanel: React.FC<ParentControlPanelProps> = ({
  ...
  // State Management
  const [state, setState] = useState<ParentControlPanelState>({
    expanded: true,
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`parentcontrolpanel_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/parentcontrolpanels/user/${userId}` : `/api/parentcontrolpanels`;
  ...
  useEffect(() => {
    if (error) {
      console.error(`ParentControlPanel error:`, error);
      onError?.(error);
    }
  }, [error, onError]);

  // Event Handlers
  const updateState = useCallback((updates: Partial<ParentControlPanelState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `parentcontrolpanel_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `parentcontrolpanel_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading parentcontrolpanels...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading parentcontrolpanels</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No parentcontrolpanels found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first parentcontrolpanel'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`parentcontrolpanel-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="parentcontrolpanel-component"
    >
