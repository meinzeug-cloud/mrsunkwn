...
export const NavigationMenu: React.FC<NavigationMenuProps> = ({
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`navigationmenu_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/navigationmenus/user/${userId}` : `/api/navigationmenus`;
  ...
  useEffect(() => {
    if (error) {
      console.error(`NavigationMenu error:`, error);
      onError?.(error);
    }
  }, [error, onError]);

  // Event Handlers
  const updateState = useCallback((updates: Partial<NavigationMenuState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `navigationmenu_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `navigationmenu_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading navigationmenus...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading navigationmenus</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No navigationmenus found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first navigationmenu'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`navigationmenu-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="navigationmenu-component"
    >
