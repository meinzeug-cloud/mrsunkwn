...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`chatinterface_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/chatinterfaces/user/${userId}` : `/api/chatinterfaces`;
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `chatinterface_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `chatinterface_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading chatinterfaces...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading chatinterfaces</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No chatinterfaces found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first chatinterface'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`chatinterface-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="chatinterface-component"
    >
