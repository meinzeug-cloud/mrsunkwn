...
export const ChartVisualization: React.FC<ChartVisualizationProps> = ({
  ...
  // State Management
  const [state, setState] = useState<ChartVisualizationState>({
    expanded: true,
  ...
  // Custom Hooks
  const [preferences, setPreferences] = useLocalStorage(`chartvisualization_preferences`, {
    viewMode: 'list',
    itemsPerPage: 20,
    sortBy: 'created_at',
    sortDirection: 'desc'
  ...
  const apiEndpoint = useMemo(() => {
    const baseUrl = userId ? `/api/chartvisualizations/user/${userId}` : `/api/chartvisualizations`;
  ...
  useEffect(() => {
    if (error) {
      console.error(`ChartVisualization error:`, error);
      onError?.(error);
    }
  }, [error, onError]);

  // Event Handlers
  const updateState = useCallback((updates: Partial<ChartVisualizationState>) => {
  ...
  const handleExport = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      downloadFile(csv, `chartvisualization_export.csv`, 'text/csv');
    } else {
      const json = JSON.stringify(exportData, null, 2);
      downloadFile(json, `chartvisualization_export.json`, 'application/json');
    }
    ...
    if (loading) {
      return (
        <div className="loading-container">
          <div className="loading-spinner">Loading chartvisualizations...</div>
        </div>
      );
    }
      ...
      return (
        <div className="error-container">
          <div className="error-message">
            <h4>Error loading chartvisualizations</h4>
            <p>{error.message}</p>
      ...
      return (
        <div className="empty-state">
          <div className="empty-message">
            <h4>No chartvisualizations found</h4>
            <p>{state.searchQuery ? 'Try adjusting your search' : 'Get started by creating your first chartvisualization'}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div 
      ref={containerRef}
      className={`chartvisualization-component theme-${theme} size-${size} variant-${variant} ${className}`}
      data-testid="chartvisualization-component"
    >
